...
#include "SimpleBoundedNDFSChecker.hh"

namespace modelChecker {

SimpleBoundedNDFSChecker::SimpleBoundedNDFSChecker(System& system, LogicFormula& property, int top):
		system(system), propertyAutomaton(&property, top)
{
...
bool
SimpleBoundedNDFSChecker::findCounterexample(int maxBound)
{
	// TODO: bounded checker..
	intersectionStates.expandTo(1);

	FOR_EACH_CONST(j, NatSet,  propertyAutomaton.getInitialStates())
	{
		if (dfs1PropertyTransitions(0, *j))
			return true;
	}
	return false;
}
...
local_inline bool
SimpleBoundedNDFSChecker::dfs1SystemTransitions(int systemStateNr, int propertyStateNr)
{
...
bool
SimpleBoundedNDFSChecker::dfs1PropertyTransitions(int systemStateNr, int propertyStateNr)
{
...
local_inline bool
SimpleBoundedNDFSChecker::dfs2PropertyTransitions(int systemStateNr, int propertyStateNr)
{
	const BuchiAutomaton2::TransitionMap& tMap =
			propertyAutomaton.getTransitions(propertyStateNr);
	BuchiAutomaton2::TransitionMap::const_iterator e = tMap.end();
...
bool
SimpleBoundedNDFSChecker::dfs2SystemTransitions(int systemStateNr, int propertyStateNr)
{
	...
	for (int i = 0;; i++)
	{
		...
		if (n == NONE)
			break;
		Assert(n < intersectionStates.length(), "visited system state for the first time on dfs2");
...
bool
SimpleBoundedNDFSChecker::satisfiesPropositionalFormula(int systemStateNr, Bdd formula)
{
